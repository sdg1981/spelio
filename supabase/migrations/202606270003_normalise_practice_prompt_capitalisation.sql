...
--
-- WARNING: Run the whole file, not an individual SELECT statement.
--
...
-- - Each audit SELECT below includes its own candidate CTE so it can be inspected
--   safely, but the intended Supabase SQL editor workflow is to run this full file.

begin;

with
changes(word_id, list_id, expected_english_prompt, normalised_english_prompt) as (
  values
...
with
skips(word_id, list_id, english_prompt, reason) as (
  values
...
with manual_review(word_id, list_id, english_prompt, reason) as (
  select
    null::text as word_id,
    null::text as list_id,
    null::text as english_prompt,
    null::text as reason
  where false
)
select
