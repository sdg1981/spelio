...
| `focus_categories` table | Legacy | Admin reference records and export snapshot structure. | Admin loaders/export still query it. |
| `word_lists.focus_category_id` | Legacy | Admin edit value and old export/import field. | Admin edit, import, export, and repository writes still include it. |
...
2. Admin word-list create/edit still writes `stage_id` and `focus_category_id`.
...
2. Remove stage/focus selectors and columns from day-to-day admin word-list editing.
